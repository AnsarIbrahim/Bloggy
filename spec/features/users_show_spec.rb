...
RSpec.feature 'User Show Page' do
  before(:each) do
    @user1 = create(:user, name: 'user1', bio: 'Teacher from Uganda', photo: 'user1_profile_image.jpg', post_counter: 0)
    @post1 = create(:post, author_id: @user1.id, title: 'Hello Everyone')
    @post2 = create(:post, author_id: @user1.id, title: 'World Everyone')
    @post3 = create(:post, author_id: @user1.id, title: 'Arc Everyone')
    @post4 = create(:post, author_id: @user1.id, title: 'Joe Everyone')
  end
