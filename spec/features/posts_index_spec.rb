...
RSpec.feature 'Post Index Page' do
  before(:each) do
    ...
    @post2 = create(:post, author_id: @user1.id, title: 'World', text: 'Hi men', comment_counter: 1, like_counter: 2)
  ...
  scenario 'I can see the number of comments on a post' do
    visit user_posts_path(@user1)
    expect(page).to have_content(@post2.comment_counter)
  end
  scenario 'I can see the number of likes a post has' do
    visit user_posts_path(@user1)
    expect(page).to have_content(@post2.like_counter)
  end
